...
/**
 * REST controller for managing {@link ar.com.ensolvers.todo.domain.File}.
 */
@RestController
@RequestMapping("/api")
@Transactional
        ...
        Optional<File> result = fileRepository
            ...
            .map(existingFile -> {
                if (file.getNombre() != null) {
                    existingFile.setNombre(file.getNombre());
                }
                if (file.getDescripcion() != null) {
                    existingFile.setDescripcion(file.getDescripcion());
                }
                if (file.getEstado() != null) {
                    existingFile.setEstado(file.getEstado());
                }
